...
export const useAIConfigStore = defineStore(`AIConfig`, () => {
  ...
  // 服务类型
  const type = persistedRef<string>(`openai_type`, DEFAULT_SERVICE_TYPE)

  // 温度参数（0-2，控制随机性）
  const temperature = persistedRef<number>(`openai_temperature`, DEFAULT_SERVICE_TEMPERATURE)

  // 最大 token 数
  const maxToken = persistedRef<number>(`openai_max_token`, DEFAULT_SERVICE_MAX_TOKEN)
  ...
  Promise.resolve().then(async () => {
    ...
    const value = await persistedGet(`openai_key_${capturedType}`)
  ...
  watch(
    ...
    async (newType) => {
      ...
      const saved = await persistedGet(`openai_model_${newType}`) || ``
      model.value = svc.models.includes(saved) ? saved : svc.models[0]

      // 保存当前模型
      await persistedSet(`openai_model_${newType}`, model.value)

      // 加载对应服务的 API Key
      const keyValue = await persistedGet(`openai_key_${newType}`)
  ...
  watch(apiKey, async (val) => {
    if (type.value !== DEFAULT_SERVICE_TYPE) {
      await persistedSet(`openai_key_${type.value}`, val)
    }
  })

  // 监听模型变化，持久化存储
  watch(model, async (val) => {
    await persistedSet(`openai_model_${type.value}`, val)
  })
  ...
  const reset = async () => {
    ...
    await Promise.all(
      serviceOptions.map(async ({ value }) => {
        await persistedRemove(`openai_key_${value}`)
        await persistedRemove(`openai_model_${value}`)
      }),
    )
  }
